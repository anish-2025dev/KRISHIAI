  ...
  try {
    ...
    try {
      ...
      if (data.records && data.records.length > 0) {
        // Group by commodity, get latest modal price
        const grouped = {};
        data.records.forEach(r => {
          const key = r.commodity?.toLowerCase();
          if (!key) return;
          if (!grouped[key] || new Date(r.arrival_date) > new Date(grouped[key].arrival_date)) {
            grouped[key] = r;
          }
        });
        ...
        const prices = Object.values(grouped).map(r => {
          ...
          const minP   = parseInt(r.min_price)    || 0;
          const maxP   = parseInt(r.max_price)    || 0;
          const base   = BASE_PRICES[r.commodity?.toLowerCase()];
          const change = parseFloat(((Math.random() - 0.45) * 6).toFixed(1));
          return {
            crop:          r.commodity,
            ...
            district:      r.district,
            current_price: modal || Math.round((minP + maxP) / 2),
            min_price:     minP,
            max_price:     maxP,
            msp:           base?.msp || null,
            unit:          'quintal',
        ...
        return res.json({ success: true, state, source: 'live', prices });
      }
    } catch (apiErr) {
      console.log('Agmarknet API failed, using base prices:', apiErr.message);
    }
    ...
    res.json({ success: true, state, source: 'estimated', prices });
  } catch (err) {
...
Current base price: Rs.${base ? (base.min + base.max) / 2 : 2000}/quintal
...
Respond ONLY with valid JSON (no markdown):
{
  ...
  "daily_forecast": [
    {"day": 1, "date": "2026-05-28", "predicted_price": 2260, "confidence": 85},
    {"day": 2, "date": "2026-05-29", "predicted_price": 2280, "confidence": 82},
    {"day": 3, "date": "2026-05-30", "predicted_price": 2300, "confidence": 78},
    {"day": 4, "date": "2026-05-31", "predicted_price": 2290, "confidence": 75},
    {"day": 5, "date": "2026-06-01", "predicted_price": 2310, "confidence": 72},
    {"day": 6, "date": "2026-06-02", "predicted_price": 2330, "confidence": 70},
    {"day": 7, "date": "2026-06-03", "predicted_price": 2350, "confidence": 68}
  ],
...
router.get('/mandis', async (req, res) => {
  try {
    ...
    const { data } = await axios.get('https://api.geoapify.com/v2/places', {
      params: {
        categories: 'commercial.market,commercial.marketplace,commercial.grocery',
    ...
    let mandis = (data.features || []).map((f, i) => ({
    ...
    if (mandis.length === 0) {
      mandis = [
        ...
        { id:'m3', name:'Shahpura Mandi',       address:'Shahpura, Jaipur',   distance:'35 km', lat:26.62, lng:75.95, timing:'6AM-1PM',  crops:['Cotton','Soybean'] },
      ];
    }
...
router.get('/search', async (req, res) => {
  try {
    const { state, commodity, district, limit = 20 } = req.query;
    const AGKEY = process.env.AGMARKNET_API_KEY;
    const AGRID = process.env.AGMARKNET_RESOURCE_ID;
...
function getCropHindi(crop) {
  const map = {
    wheat:'गेहूं', rice:'चावल', maize:'मक्का', mustard:'सरसों',
    soybean:'सोयाबीन', cotton:'कपास', onion:'प्याज', potato:'आलू',
    tomato:'टमाटर', chickpea:'चना', bajra:'बाजरा', jowar:'ज्वार',
    groundnut:'मूंगफली', sugarcane:'गन्ना', turmeric:'हल्दी',
  };
